...
class lipEmbeddingExtractor(nn.Module):
    ...
    def forward(self, inputBatch):
        ...
        # print('shape',outputBatch.shape) #[B,length,embedding]
...
class Separation(nn.Module):
    ...
    def forward(self,audio_em,video_em):
        ...
        # print('video_emb ',video_em.shape) #b,512,length 

        video_em =  F.interpolate(video_em, size=audio_em.shape[2], mode='linear')

        concat = torch.cat((audio_em,video_em),dim=1) 
        #The shape of video_emb is 256 in AVConv_tasnet paper,but its pre-trained 
        # lip embedding extractor in the reference paper is 512 , i guess wujian makes 
        # a mistak when descrpting the dimension. I adopt 512 here.  
...
#DAVSE
class DAVSE(nn.Module):
    
    def __init__(self,):
        super(DAVSE,self).__init__()

        self.audioencoder = AudioEncoder()

        #video encoder 
        self.IDextractor = lipEmbeddingExtractor()  # speaker ID extractor
        self.SemanticExtractor = lipEmbeddingExtractor() # syncronization extractor 

        #load ID_extractor
        model_dict = self.IDextractor.state_dict()
        checkpoint = torch.load('../pretrainedmodel/AV-ConvTasNet-Spk_face.pt')
        ...
        self.IDextractor.load_state_dict(model_dict)
        for k, v in self.IDextractor.named_parameters():
            v.requires_grad=False

        #load synchronizaiton 
        model_dict = self.SemanticExtractor.state_dict()
        checkpoint = torch.load('../pretrainedmodel/AV-ConvTasNet-Sync_face.pt')
        pretrained_model_dict = checkpoint['model']
        for k,v in model_dict.items():
            if 'module.visualencoder.lipembeddingextractor.'+k in pretrained_model_dict.keys():
                model_dict[k]=pretrained_model_dict['module.visualencoder.lipembeddingextractor.'+k]
                ...
                print('not load '+str(k))
        self.SemanticExtractor.load_state_dict(model_dict)
        for k, v in self.SemanticExtractor.named_parameters():
            v.requires_grad=False
        ...
        self.linear = nn.Conv1d(256*2,256, 1,1)
    ...
    def forward(self,audio,video):
        ...
        ID_feature = self.IDextractor(video) # B embedding Length 
        Semantic_feature = self.SemanticExtractor(video)

        audio_em = self.Conv1D_S_pre(mix_audio_emb)
        ID_feature =  F.interpolate(ID_feature, size=audio_em.shape[2], mode='linear')
        Semantic_feature =  F.interpolate(Semantic_feature, size=audio_em.shape[2], mode='linear')


        # fused = self.att(audio_em,ID_feature,Semantic_feature)
        fused = torch.cat((ID_feature,Semantic_feature),dim=1)
        fused = self.linear(fused)

        out = self.net(fused)


        concat = torch.cat((audio_em,out),dim=1)
        projected = self.project(concat)

        est_mask_emb = self.Conv1D_S_post(projected)
